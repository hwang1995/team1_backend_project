...
public class NoticeServiceImpl implements INoticeService {
  ...
  /**
   * 공지사항 게시물을 추가하기 위한 서비스 메서드 시나리오 : 
   * 1-1. 요청받은 noticeInfo.isNull가 true인 경우 BadRequestException을 처리해준다.
   * 1-2. 비즈니스 로직 중 불가능하거나 모순이 생긴 경우 ConflictRequestException을 처리해준다. 
   * 2-1. 올바른 요청이 들어왔을 경우, noticesDAO.insertNotice의 값이 1이 아니 result = false를 리턴해준다. 
   * 2-2. 올바른 요청이 들어왔을 경우, noticesDAO.insertNotice의 값이 1일 경우 result = true를 리턴해준다.
  ...
  /**
   * 공지사항 게시물을 제거하기 위한 서비스 메서드 시나리오 : 
   * 1-1. 요청받은 noticeId이 null인 경우 BadRequestException을 처리해준다. 
   * 2-1. 올바른 요청이 들어왔을 경우, noticesDAO.deleteByNoticeId의 값이 1이 아닐 경우 result = false를 리턴해준다. 
   * 2-2. 올바른 요청이 들어왔을 경우, noticesDAO.deleteByNoticeId의 값이 1일 경우 result = true를 리턴해준다.
   * @param : int noticeId
  ...
  /**
   * 공지사항 게시물을 수정하기 위한 서비스 메서드 시나리오 :
   * 1-1. 요청받은 noticeInfo.isNull의 값이 true인 경우 BadRequestException을 처리해준다.
   * 2-1. 올바른 요청이 들어왔을 경우, noticesDAO.updateNotice의 값이 1이 아닐 경우 result = false를 리턴해준다. 
   * 2-2. 올바른 요청이 들어왔을 경우, noticesDAO.updateNotice의 값이 1일 경우 result = true를 리턴해준다.
   * @param : NoticesDTO noticeInfo
  ...
  /**
   * 해당 병원의 공지사항 게시물 리스트를 가져오기 위한 서비스 메서드 시나리오 : 
   1-1. 요청받은 String hospitalCode의 값이 null인 경우 BadRequestException을 처리해준다. 
   2-1. 올바른 요청이 들어왔지만 , noticesDAO.selectNoticesByHospitalCode의 값이 없어서 불러올 수 없는 경우 NoContentException을 처리해준다.
   2-2. 올바른 요청이 들어왔고, noticesDAO.selectNoticesByHospitalCode의 값이 있지만 불러올 수 없는 경우 NotFoundException을 처리해준다.
   2-2. 올바른 요청이 들어왔고, noticesDAO.selectNoticesByHospitalCode의 값이 제대로인 경우 notices를 리턴해준다.
  ...
  /**
   * 해당 병원의 제목을 포함한 공지사항 게시물 리스트를 가져오기 위한 서비스 메서드 시나리오 :
   1-1. 요청받은 searchNoticeByHospitalAndTitle.isNull의 값이 true인 경우 BadRequestException을 처리해준다. 
   2-1. 올바른 요청이 들어왔지만 , noticesDAO.selectNoticesByHospitalCodeAndTitle(searchNoticeByHospitalAndTitle)의 값이 없어서 불러올 수 없는 경우 NoContentException을 처리해준다.
   2-2. 올바른 요청이 들어왔고, noticesDAO.selectNoticesByHospitalCodeAndTitle(searchNoticeByHospitalAndTitle)의 값이 있지만 불러올 수 없는 경우 NotFoundException을 처리해준다.
   2-2. 올바른 요청이 들어왔고, noticesDAO.selectNoticesByHospitalCodeAndTitle(searchNoticeByHospitalAndTitle)의 값이 제대로인 경우 notices를 리턴해준다.
  ...
  /**
   * 공지사항 게시물의 상세 정보를 가져오기 위한 서비스 메서드 시나리오 :
   * 요청받은  noticeId의 값이 null인 경우 BadRequestException을 처리해준다.
   * 2-1. 올바른 요청이 들어왔을 경우, noticesDAO.selectNoticeDetailByNoticeId(noticeId) 값이 값이 없어서 불러올 수 없는 경우 NoContentException을 처리해준다.
   * 2-2. 올바른 요청이 들어왔고, noticesDAO.selectNoticeDetailByNoticeId(noticeId)의 값이 있지만 불러올 수 없는 경우 NotFoundException을 처리해준다.
   * 2-3. 올바른 요청이 들어왔고, noticesDAO.selectNoticeDetailByNoticeId(noticeId)의 값이 제대로인 경우 notice를 리턴해준다.
   * @param : int noticeId
  ...
  /**
   * 공지사항 게시물의 조회수를 수정하기 위한 서비스 메서드 시나리오 :
   * 1-1. 요청받은  noticeId의 값이 null인 경우 BadRequestException을 처리해준다.
   * 2-1. 올바른 요청이 들어왔을 경우, noticesDAO.updateNoticeCountByNoticeId(noticeId)의 값이 1이 아닐 경우 result = false를 리턴해준다. 
   * 2-2. 올바른 요청이 들어왔을 경우, noticesDAO.updateNoticeCountByNoticeId(noticeId)의 값이 1일 경우 result = true를 리턴해준다.
   * @param : int noticeId
  ...
  /**
   * 게시물의 식별자 (ID)로 댓글 리스트를 조회하기 위한 서비스 메서드 시나리오 :
   * 1-1. 요청받은  noticeId의 값이 null인 경우 BadRequestException을 처리해준다.
   * 2-1. 올바른 요청이 들어왔을 경우, noticeCommentsDAO.selectCommentsByNoticeId(noticeId) 값이 값이 없어서 불러올 수 없는 경우 NoContentException을 처리해준다.
   * 2-2. 올바른 요청이 들어왔고, noticeCommentsDAO.selectCommentsByNoticeId(noticeId)의 값이 있지만 불러올 수 없는 경우 NotFoundException을 처리해준다.
   * 2-3. 올바른 요청이 들어왔고, noticeCommentsDAO.selectCommentsByNoticeId(noticeId)의 값이 제대로인 경우 noticeComments를 리턴해준다.
   * @param : int noticeId
  ...
  /**
   * 게시물을 추가하기 위한 서비스 메서드 시나리오 : 
   * 1-1. 요청받은 noticeCommentInfo.isNull가 true인 경우 BadRequestException을 처리해준다. 
   * 2-1. 올바른 요청이 들어왔을 경우, noticeCommentsDAO.insertComment의 값이 1이 아니 result = false를 리턴해준다. 
  ...
  /**
   * 댓글을 삭제하기 위한 서비스 메서드 시나리오 : 
   * 1-1. 요청받은 noticeCommentsId이 null인 경우 BadRequestException을 처리해준다.
   * 2-1. 올바른 요청이 들어왔을 경우, noticeCommentsDAO.deleteComment의 값이 1이 아닐 경우 result = false를 리턴해준다. 
   * 2-2. 올바른 요청이 들어왔을 경우, noticeCommentsDAO.deleteComment의 값이 1일 경우 result = true를 리턴해준다.
  ...
  /**
   * 덧글을 수정하기 위한 서비스 메서드 시나리오 : 
   * 1-1. 요청받은 updateCommentInfo.isNull의 값이 true인 경우 BadRequestException을
  ...
  /**
   * 게시물에 사진을 첨부하기 위한 서비스 메서드 :
   * 1-1. 요청받은 noticeImageInfo.isNull의 값이 true인 경우 BadRequestException을 처리해준다.
   * 2-1. 올바른 요청이 들어왔을 경우, noticeImageInfo을 리턴해준다.
